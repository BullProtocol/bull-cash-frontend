...
import { BOND_REDEEM_PRICE, BOND_REDEEM_PRICE_BN } from '../../bcash/constants';
...
const Bond: React.FC = () => {
  ...
  const bondBalance = useTokenBalance(basisCash?.BBond);
  ...
  const handleBuyBonds = useCallback(
    async (amount: string) => {
      ...
      addTransaction(tx, {
        summary: `Buy ${bondAmount.toFixed(2)} BBond with ${amount} BCASH`,
      });
    },
          ...
          <>
            ...
            <StyledBond>
              <StyledCardWrapper>
                <ExchangeCard
                  action="Purchase"
                  fromToken={basisCash.BCASH}
                  fromTokenName="Bcash"
                  toToken={basisCash.BBond}
                  toTokenName="Bbond"
                  priceDesc={
                    !isBondPurchasable
                      ? 'BCASH is over $1'
              ...
              <StyledStatsWrapper>
                ...
                <ExchangeStat
                  tokenName="BBond"
                  description="Current Price: (BCASH)^2"
              ...
              <StyledCardWrapper>
                <ExchangeCard
                  action="Redeem"
                  fromToken={basisCash.BBond}
                  fromTokenName="Bbond"
                  toToken={basisCash.BCASH}
                  toTokenName="Bcash"
                  priceDesc={`${getDisplayBalance(bondBalance)} BBond Available`}
                  onExchange={handleRedeemBonds}
                  disabled={!bondStat || bondBalance.eq(0) || !isBondRedeemable}
                  disabledDescription={!isBondRedeemable ? `Enabled when BCASH > $${BOND_REDEEM_PRICE}` : null}
                />
              </StyledCardWrapper>
            </StyledBond>
          </>
