...
/**
 * An API module of Bcash contracts.
...
export class BasisCash {
  ...
  constructor(cfg: Configuration) {
    ...
    this.BCASH = new ERC20(deployments.Cash.address, provider, 'BCASH');
    this.BSHARE = new ERC20(deployments.Share.address, provider, 'BSHARE');
    this.BBond = new ERC20(deployments.Bond.address, provider, 'BBond');

    // Uniswap V2 Pair
    this.bacDai = new Contract(
      externalTokens['BCASH_BUSD-CAKE-LPv2'][0],
      IUniswapV2PairABI,
      provider,
    );
  ...
  unlockWallet(provider: any, account: string) {
    ...
    const tokens = [this.BCASH, this.BSHARE, this.BBond, ...Object.values(this.externalTokens)];
  ...
  /**
   * @returns Bcash (BCASH) stats from Uniswap.
   * It may differ from the BCASH price used on Treasury (which is calculated in TWAP)
   */
  async getCashStatFromUniswap(): Promise<TokenStat> {
    const supply = await this.BCASH.displayedTotalSupply();
    return {
      priceInDAI: await this.getTokenPriceFromUniswap(this.BCASH),
      totalSupply: supply,
    };
  }

  /**
   * @returns Estimated Bcash (BCASH) price data,
  ...
  async getCashStatInEstimatedTWAP(): Promise<TokenStat> {
    ...
    const totalSupply = await this.BCASH.displayedTotalSupply();
  ...
  async getBondStat(): Promise<TokenStat> {
    ...
    return {
      ...
      totalSupply: await this.BBond.displayedTotalSupply(),
    };
  }

  async getShareStat(): Promise<TokenStat> {
    return {
      priceInDAI: await this.getTokenPriceFromUniswap(this.BSHARE),
      totalSupply: await this.BSHARE.displayedTotalSupply(),
    };
  }
  ...
  async fetchBoardroomVersionOfUser(): Promise<string> {
    ...
    if (balance1.gt(0)) {
      console.log(
        `👀 The user is using Boardroom v1. (Staked ${getDisplayBalance(balance1)} BSHARE)`,
      );
      return 'v1';
    }
    ...
    if (balance2.gt(0)) {
      console.log(
        `👀 The user is using Boardroom v2. (Staked ${getDisplayBalance(balance2)} BSHARE)`,
      );
      return 'v2';
    }
    return 'latest';
